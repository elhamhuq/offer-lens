...
export default function AnimatedPortfolioChart({ className = '' }: AnimatedPortfolioChartProps) {
  ...
  const generateAreaPath = (data: typeof data, progress: number) => {
    ...
    const lastX = ((Math.ceil((data.length * progress) / 100) - 1) / (data.length - 1)) * 100;
  ...
  const getCurrentValue = () => {
    ...
    const progressIndex = Math.ceil((data.length * animationProgress) / 100) - 1;
      ...
      >
        ...
        <path
          ...
          style={{
            ...
            animation: animationProgress === 100 ? 'drawLine 2s ease-out forwards' : 'none'
          }}
        />
