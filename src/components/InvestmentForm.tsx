...
export default function InvestmentForm({
  ...
  const updateInvestmentAmount = (id: string, amount: number) => {
    const updatedInvestments = investments.map(inv =>
      inv.id === id ? { ...inv, monthlyAmount: amount } : inv
    );
    setInvestments(updatedInvestments);
    onInvestmentsChange?.(updatedInvestments);
  };
      ...
      <CardContent className='space-y-6'>
        ...
        <div className='grid md:grid-cols-3 gap-4'>
          ...
          <Card className='bg-muted/30 border-border'>
            <CardContent className='p-4 text-center'>
              ...
              <p
                className={`text-lg font-bold ${remainingBudget < 0 ? 'text-destructive' : 'text-foreground'}`}
              >
        ...
        <div className='space-y-4'>
          <div className='flex items-center justify-between'>
            ...
            <Badge
              variant='secondary'
              className={riskLevels[getOverallRiskLevel()].bg}
            >
                  ...
                  <CardContent className='p-4'>
                    ...
                    <div className='space-y-2'>
                      ...
                      <Slider
                        ...
                        onValueChange={([value]) =>
                          updateInvestmentAmount(investment.id, value)
                        }
                        max={remainingBudget + investment.monthlyAmount}
                        min={100}
                        step={50}
                        className='w-full'
                      />
                      <div className='flex justify-between text-xs text-muted-foreground'>
                        ...
                        <span>
                          $
                          {(
                            remainingBudget + investment.monthlyAmount
                          ).toLocaleString()}
                        </span>
                      </div>
                    </div>
          ...
          <CardContent className='space-y-4'>
            ...
            <div className='space-y-2'>
              <div className='flex justify-between text-sm'>
                ...
                <span
                  className={`font-medium ${riskLevels[getOverallRiskLevel()].color}`}
                >
        ...
        {remainingBudget < 0 && (
          <Alert variant='destructive'>
            ...
            <AlertDescription>
              Your investments exceed your monthly budget by $
              {Math.abs(remainingBudget).toLocaleString()}. Consider reducing
              some allocations.
            </AlertDescription>
          </Alert>
        )}
        ...
        {investments.length === 1 && (
          <Alert>
            ...
            <AlertDescription>
              Consider diversifying your portfolio by adding investments in
              different asset classes or geographic regions.
            </AlertDescription>
          </Alert>
        )}
        ...
        {budgetUtilization < 50 && (
          <Alert>
            ...
            <AlertDescription>
              You're only using {budgetUtilization.toFixed(1)}% of your
              investment budget. Consider increasing your contributions to
